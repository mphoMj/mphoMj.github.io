...
function amort(customerName, loanAmount, loanInterest, loanDate, dueDate, loanTerm) {
    ...
    $('#Result').append(
      ...
      //  "<div>Monthly payment: MK " + monthlyPrincipal.toFixed(2) + "</div>" +
        ...
        "<tr><td>Repayment Due : </td><td>" + dueDatein.toLocaleDateString("en-GB", options) + "</td></tr></table></div>" //+
      //  "<div>Total to be paid: MK " + (totalRepayment.toFixed(2)).toLocaleString() +
    );

    //add header row for table to return string
    var repaymentTable ="<table >" +
    "<tr><thead><th></th><th>Payment Date </th><th>Interest</th>" +
    ...
    for (var count = 0; count < loanTerm; count++) {
        ...
        repaymentTable +="<td> MK " + interestDue.toFixed(2) + "</td>";
                ...
                repaymentTable +="<td> MK " + reducingBalance.toFixed(2) + "</td>";

        //the payment due at a particular iteration
        totalDue = monthlyPrincipal + interestDue;
        repaymentTable +="<td> MK " + totalDue.toFixed(2) + "</td>";

        //increment the total amount due
